...
bool KJpsiToElElProducer::filter(edm::Event& event, const edm::EventSetup& eventSetup)
{
  ...
  edm::Handle< std::vector<pat::Electron>  > leptonHandle;
  ...
  for ( int i=0, n=leptonHandle->size(); i<n; ++i )
  {
    ...
    for ( int j=0; j<n; ++j )
    {
      ...
      */
			...
			ROOT::Math::LorentzVector<PxPyPzE4D<double> > sum_vec = lep1.p4() + lep2.p4();
			
      //const double rawEnergy = sum_vec.E() ;
      ...
      // Match to muons
      pat::Electron newLep1(lep1);
      pat::Electron newLep2(lep2);
